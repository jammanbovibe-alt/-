    ...
    <div className="flex h-screen ios-wallpaper font-sans text-slate-100 overflow-hidden p-6 gap-6">
      ...
      <aside className="w-72 ios-glass rounded-[32px] flex flex-col shrink-0 overflow-hidden shadow-2xl relative">
        ...
        <div className="p-8 flex items-center gap-3.5 cursor-pointer border-b border-white/5" onClick={() => navigate('/')}>
          <div className="w-10 h-10 bg-gradient-to-br from-amber-500 via-orange-500 to-rose-500 rounded-2xl flex items-center justify-center font-bold text-white shadow-lg shadow-orange-500/20 glow-orange">
            <Sparkles size={20} className="text-white" />
          </div>
          <div className="flex flex-col">
            <span className="text-white font-extrabold text-lg tracking-tight leading-tight">JamClass</span>
            <span className="text-[10px] text-orange-300/70 font-semibold tracking-wider uppercase">AI 잼봇 v2.0</span>
          </div>
        </div>
        
        {/* Navigation Links */}
        <nav className="flex-1 px-4 py-8 space-y-2.5">
          <NavItem to="/" icon={LayoutDashboard} label="모니터링 대시보드" />
          <NavItem to="/join" icon={Users} label="학생 접속 (미리보기)" />
          <NavItem to="#" icon={BookOpen} label="복습 자료실 (준비중)" disabled />
        </nav>
        ...
        <div className="p-6 mt-auto">
          <div className="bg-white/5 border border-white/5 rounded-2xl p-4.5 flex items-center gap-3.5 shadow-sm">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <p className="text-[10px] text-orange-300/60 font-semibold tracking-tight mt-0.5">초등 교육 전문가</p>
            </div>
